...
namespace RoomManager
{
    public partial class Dat_TraPhong : Form
    {
        ...
        private void BNT_datphong_Click(object sender, EventArgs e)
        {
            TimeSpan duration_Day = datePick_tra_datphong.Value.Date - datePick_dat_datphong.Value.Date;
            int firstTotalDay_traphong = duration_Day.Days;
            DateTime checkin = datePick_dat_datphong.Value;
            DateTime checkout = datePick_tra_datphong.Value;
            if (firstTotalDay_traphong > 0) 
            {
                ...
                {
                    int roomId = Convert.ToInt32(Grid_datphong.SelectedRows[0].Cells[0].Value.ToString());
                    int httpStatus = customerController.CheckInRoom(checkin, checkout, roomId, auth.GetUserId(username));
                    if (httpStatus == 200)
                    {
                        //success
                        // update room status to booked
                        roomController.UpdateStatus(roomId, "Đã đặt");
                        //show message
            ...
            {
                MessageBox.Show("Bạn hãy chọn đúng lại ngày đặt phòng!", "Thông báo lỗi", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        private void BTN_traphong_Click(object sender, EventArgs e)
        {
            ...
            {
                int httpStatus = customerController.CheckOutRoom(username, roomId);
                //update room status to not booked
                if (httpStatus == 200)
                {
                    roomController.UpdateStatus(roomId, "Chưa đặt");
